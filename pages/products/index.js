  ...
  return (
    <div>
      ...
      <main className={styles.productsMain}>
        ...
        <select name="sort" value={sort} id="sort-dropdown" onChange={e => setSort(e.target.value)}>
          <option value="reviewRatingStatistics.averageRating asc">Reviews, ascending</option>
          <option value="reviewRatingStatistics.averageRating desc">Reviews, descending</option>
        </select>

        <label for="filter">Sort by:</label>
        <select name="filter" value={filter} id="filter-dropdown" onChange={e => setFilter(e.target.value)}>
              ...
              <div className={styles.product}>
                ...
                <h3>{currentData.name["en-US"] || currentData.name["en"]}</h3>
                  {item.reviewRatingStatistics?.averageRating &&
                    <h4>
                      {"Average Review: "}
                      {[...Array(Math.floor(item.reviewRatingStatistics.averageRating)).keys()].map((i) => (
                        <span>&#x2b50;</span>
                      ))}
                    </h4>
                }
                
                {!item.reviewRatingStatistics?.averageRating && <h4>No Reviews</h4>}
              </div>
            )
          }
          )}
